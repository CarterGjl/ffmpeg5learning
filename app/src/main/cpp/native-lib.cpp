...
Java_com_example_ffmpeg5learning_MainActivity_ffmpegVersion(JNIEnv *env, jobject thiz) {
    ...
    //获取avutil三个子版本号
    auto a = version / (int) pow(2, 16);
    auto b = (int) (version - a * pow(2, 16)) / (int) pow(2, 8);
    auto c = version % (int) pow(2, 8);
    //拼接avutil完整版本号
    char *charArray = new char[1024];
//    std::stringstream() << a << "." << b << "." << c << '\0' >> charArray;
    char str[25];
    auto mclass1 = env->GetObjectClass(thiz);
    jmethodID  mid = env->GetMethodID(mclass1,"callback", "(Ljava/lang/String;)V");
